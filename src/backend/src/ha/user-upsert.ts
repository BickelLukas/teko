 ...
 * soft-deactivated, and updates name when HA data changes.
 * User-owned preferences (locale, theme, display_name, etc.) are never touched.
 */
export function upsertHaUser(db: Db, haUserId: string, name: string): UpsertResult {
  const existing = db
    .select()
    .from(schema.users)
  ...
  if (!existing) {
    ...
    db.insert(schema.users)
      .values({ id, ha_user_id: haUserId, name, is_admin: false, is_active: true })
      .run();
  ...
  if (wasInactive || nameChanged) {
    db.update(schema.users)
      .set({ name, is_active: true })
      .where(eq(schema.users.ha_user_id, haUserId))
      .run();
    const user: UserRow = { ...existing, name, is_active: true };
